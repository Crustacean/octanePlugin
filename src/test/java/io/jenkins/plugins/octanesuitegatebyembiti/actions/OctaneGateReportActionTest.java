...
public class OctaneGateReportActionTest {
  ...
  public void attachesToBuildAndRendersReportPage() throws Exception {
    ...
    assertTrue(xml.contains("stroke: var(--octane-color-bad)"));
    ...
    assertTrue(xml.contains("TIMER_HEAD_RADIUS = 8"));
    assertTrue(xml.contains("TIMER_ACTIVE_OPACITY = \"1\""));
    assertTrue(xml.contains("extendedProgress <= 0 ? \"0\" : TIMER_ACTIVE_OPACITY"));
    assertTrue(xml.contains("--octane-timer-head-fill: #000000"));
    assertTrue(xml.contains("--octane-timer-head-fill: #ffffff"));
    assertTrue(xml.contains("--octane-timer-head-gap: var(--background)"));
    assertTrue(xml.contains("fill: var(--octane-timer-head-fill)"));
    assertTrue(xml.contains("stroke: var(--octane-timer-head-gap)"));
    assertTrue(xml.contains("stroke-width: 1.5px"));
    assertTrue(xml.contains("r=\"8\""));
    assertFalse(xml.contains("r=\"6\""));
    assertFalse(xml.contains("TIMER_HEAD_BUFFER_RADIUS"));
    assertFalse(xml.contains("state.headBuffer"));
    assertTrue(xml.contains("state.head.setAttribute(\"r\", String(TIMER_HEAD_RADIUS))"));
    assertFalse(xml.contains("state.head.setAttribute(\"fill\""));
    assertTrue(xml.contains("state.mode === \"timeout\" && state.extendedActive"));
    ...
    assertTrue(xml.contains("function initialAutoFlipBoundaryReached"));
    ...
    assertTrue(xml.contains("snapshotProgress(payload) >= 100"));
  ...
  public void extendedTimeReportShowsManualExitControl() throws Exception {
    ...
    assertTrue(xml.contains("aria-hidden=\"true\""));
    assertTrue(xml.contains("fill: var(--octane-timer-head-fill)"));
    assertTrue(xml.contains("stroke-width: 1.5px"));
