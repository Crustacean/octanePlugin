...
| Pipeline | Persisted one-poll state machine; Jenkins Timer only schedules short wake-ups; HTTP/poll work runs on a dedicated 32-thread daemon pool with a 256-task queue |
...
| Worker preservation | No Timer/CPS worker blocked across HTTP or poll intervals | Timer schedules wake-ups only; the bounded poll pool owns poll/HTTP waits |
...
| Correctness | Criteria results identical to the legacy path for all fixtures | Existing criteria/runner suite retained; all 199 Maven tests pass |
| Resilience | Restart/cancel/401/429/partial-page tests pass without duplicate completion | Resume/cancel/401/retry/paging coverage retained; all 199 Maven tests pass |
